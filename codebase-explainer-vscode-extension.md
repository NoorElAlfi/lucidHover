...
12. **Language support is declared once, in the language manifest** (Sessions 20-22, CLAUDE.md's Core Rule 12). Language-specific data — extensions, grammar, query file, exclusion patterns, resolution strategy — lives in `languages.json` and per-language query files, not scattered across extension-host code. Language-specific *logic* lives only in sidecar adapter implementations (the Language Adapter interface, Session 21). The extension host reads the manifest and contains no per-language branching of its own.
