 ...
 *
 * The launched instance opens `fixtures/sample-repo` as its workspace
 * folder (already trusted -- see `.vscode-test/user-data` seeding below),
 * matching what every prior session's manual verification used.
 */
...
async function main(): Promise<void> {
    ...
    const workspacePath = path.resolve(extensionDevelopmentPath, 'fixtures', 'sample-repo');
